...
for (var i = 0; i < arr.length; i++) {
  ...
  arr[i].classList.add("animate__zoomInDown"); //必要项，添加样式动画
}
// 给侧栏卡片套上动画
var arr = document.getElementsByClassName("card-widget");
for (var i = 0; i < arr.length; i++) {
  arr[i].classList.add("wow");
  arr[i].classList.add("animate__bounceInDown");  // animate__slideInRight
}
// 给友链卡片套上动画
var arr = document.getElementsByClassName("flink-list-item");
for (var i = 0; i < arr.length; i++) {
  arr[i].classList.add("wow");
  arr[i].classList.add("animate__flipInX");  // animate__slideInRight
}
